...
describe("DowgoERC20 - roles", function () {
  ...
  beforeEach(async () => {
    ({ dowgoERC20, usdcERC20, addr1, addr2, addr3, dowgoAdmin } = await setupTestEnvDowgoERC20({
      initialPrice,
      initialUSDCReserve,
      initialUser1USDCBalance,
      mockUSDCSupply,
  ...
  it("Should check that user1 is whitelisted", async function () {
    expect(await dowgoERC20.hasRole(WHITELISTED_ROLE, addr1.address), "role change wasn't enacted");
  });
  ...
  it("Should add user 1 as admin and whitelisted user", async function () {
    const addAdminUser1Tx = await dowgoERC20.connect(dowgoAdmin).grant_admin(addr1.address);
  ...
  it("Should not be able to add non whitelisted as admin", async function () {
    try {
      const addAdminUser3Tx = await dowgoERC20.connect(dowgoAdmin).grant_admin(addr3.address);
  ...
  it("Should not be able to add to whitelist if not admin", async function () {
    try {
      const addAdminUser3Tx = await dowgoERC20.connect(addr2).whitelist(addr3.address);
      await addAdminUser3Tx.wait();
    } catch (e: any) {
      expect(e.toString()).to.equal(
