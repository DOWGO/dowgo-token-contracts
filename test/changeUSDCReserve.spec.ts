...
describe("DowgoERC20 - USDC Reserve", function () {
  ...
  describe("DowgoERC20 - increase_usdc_reserve", function () {
    it("Should let admin address increase usdc reserve", async function () {
      await approveTransfer(usdcERC20, dowgoAdmin, dowgoERC20.address, testUSDCAmount);
      const increaseTx = await dowgoERC20.connect(dowgoAdmin).increase_usdc_reserve(testUSDCAmount);

      // wait until the transaction is mined
      await increaseTx.wait();

      // Check that reserve has been increased
      expect(await dowgoERC20.totalUSDCReserve()).to.equal(testUSDCAmount.add(initialUSDCReserve));
      ...
      expect(events[0] && events[0].args[1] && events[0].args[1]).to.equal(testUSDCAmount);
    });
    it("Should not let non-admin address increase usdc reserve", async function () {
      try {
        ...
        const increaseTx = await dowgoERC20.connect(addr1).increase_usdc_reserve(testUSDCAmount);
  ...
  describe("DowgoERC20 - decrease_usdc_reserve", function () {
    ...
    it("Should let admin address decrease usdc reserve", async function () {
      const decreaseTx = await dowgoERC20.connect(dowgoAdmin).decrease_usdc_reserve(SMALL_DECREASE);

      // wait until the transaction is mined
      await decreaseTx.wait();

      // Check that reserve has been decreased
      expect(await dowgoERC20.totalUSDCReserve()).to.equal(initialUSDCReserve.sub(SMALL_DECREASE));
    ...
    it("Should not let non-admin address decrease usdc reserve", async function () {
      try {
        const decreaseTx = await dowgoERC20.connect(addr1).decrease_usdc_reserve(testUSDCAmount);
    ...
    it("Should not let admin address decrease usdc reserve bellow min ratio", async function () {
      ...
      const USDC_COST_NO_FEE = TEST_AMOUNT.mul(initialPrice).div(ONE_DOWGO_UNIT);
      ...
      // Check that reserve has NOT been decreased
      expect(await dowgoERC20.totalUSDCReserve()).to.equal(initialUSDCReserve.add(initialPrice));
