...
describe("DowgoERC20 - withdraw", function () {
  ...
  let addr2:SignerWithAddress
    ...
    beforeEach(async()=>{
      ({dowgoERC20,addr2,usdcERC20}=await setupTestEnv())
    })
    it("Should not let user withdraw with no balance", async function () {
      
        try {
            const decreaseTx = await dowgoERC20.connect(addr2).withdraw_usdc(ONE_UNIT);
      
            // wait until the transaction is mined
            await decreaseTx.wait();
          } catch(e:any){
            expect(e.toString()).to.equal(`Error: VM Exception while processing transaction: reverted with reason string 'User doesn't have that much usdc credit'`)
          }
