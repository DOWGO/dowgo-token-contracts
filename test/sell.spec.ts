...
import { initialDowgoSupply, initialPrice, initialUSDCReserve, initialUser1USDCBalance, initPriceInteger, ONE_UNIT } from "./test-constants";
...
describe("DowgoERC20 - sell", function () {
  ...
  const SELL_AMOUNT=ONE_UNIT
    ...
    beforeEach(async()=>{
      ...
      await approveTransfer(usdcERC20,addr1,dowgoERC20.address,SELL_AMOUNT.mul(2))
      // buy
    ...
    it("Should not let user 1 sell too much tokens (more than 3%*10%=0.3% of total supply =3DWG)", async function () {
      ...
      try {
        // Approve erc20 transfer
        await approveTransfer(usdcERC20,addr1,dowgoERC20.address,SELL_AMOUNT_TOO_HIGH.mul(initPriceInteger))
        
        // Create buy tx
        const buyTx = await dowgoERC20.connect(addr1).buy_dowgo(SELL_AMOUNT_TOO_HIGH);
      ...
      expect(await dowgoERC20.totalUSDCSupply()).to.equal(initialUSDCReserve.add(SELL_AMOUNT.mul(2)));
